...
const askFn = async (question) => {
  ...
  const res = await axios.post('/search/documents', {
    doi: doi.value,
    question,
  })
  return {
...
const searchSelectionHandler = async () => {
  axios.defaults.baseURL = '/web'
  const res = await axios.post('/article/overview/analysis', {
...
<template>
  <div class="page-container">
    ...
    <div v-show="selectionMenu.show" :style="{
      ...
      <div>
        <ElButton @click="translateSelectionHandler" size="small">翻译</ElButton>
      </div>
    </div>
